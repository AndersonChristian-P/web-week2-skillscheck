...
let faveColors = ["purple", "green", "orange"];
...
let me = {
  firstName: "Christian",
  superHeroName: "Mr. Incredilicious",
  homeTown: "Kodiak",
  superPowers: ["super strength", "cosmic laser beams", "flight"],
...
function setColor(faveColors) {
  let color1 = faveColors.splice(0, 1);
  let color2 = faveColors.splice(faveColors[1], 1);
  let color3 = faveColors.splice(faveColors[2], 1);
...
let myPowers = me["superPowers"]

function setPowers(myPowers) {
  for (let i = 0; i < myPowers.length; i++) {
    createLi(myPowers[i]);
  }
}
